...
| [hackernews-show-hn](./hackernews-show-hn/SKILL.zh.md) | 准备或 troubleshoot 一个 Hacker News Show HN 帖子。覆盖新账号的静默限流（`/showlim` 重定向）、karma 门槛、1–2 周账号 warming 计划、帖子模板、HN 冷却期可同步推的备用渠道（Reddit / Twitter / awesome-* 等）。内置 pg 的 Show HN 官方规则、HN 社区通用规则、throttle 页面的本地副本 |
